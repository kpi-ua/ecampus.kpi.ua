...
export const LecturerList = ({ lecturers, selectedEmployeeId, onSelect }: Props) => {
  ...
  return (
    <section className="w-full overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-sm lg:w-[390px] lg:shrink-0">
      <div className="p-4">
        <Input
          aria-label={t('search')}
          icon={<Search />}
        ...
        {paginatedItems.map((lecturer) => {
          ...
          return (
            <button
              ...
              className={cn(
                'flex w-full items-center gap-3 border-r-2 border-transparent px-4 py-3 text-left transition-colors',
                'hover:bg-brand-00 disabled:cursor-default disabled:opacity-60',
                isSelected && 'border-r-basic-blue bg-brand-00',
              )}
            >
              <ProfilePicture size="xs" src="" />
              <span className="min-w-0 flex-1 truncate text-sm font-medium text-neutral-900">{lecturer.fullName}</span>
