        ...
        <div className="card">
          <div className="card-body">
            <div className="text-center">
              <a href="/"> <img src="/images/logo-big-green.png" alt="Електроний кампус" className="img-responsive logo-green"/> </a>
              <h2 className="text-center">Авторизацiя через KPI ID</h2>
              <div className="panel-body">
                <form>
                  <fieldset>
                    <div className="form-group">
                      <input type="text" value={this.state.login} onChange={this.setPhone} className="form-control" placeholder="Телефон у форматi 095XXXXXXX"/>
                    </div>

                    {this.state.codeWasRequested === true &&
                    <div className="form-group">
                      <input type="text" maxLength="4" value={this.state.password} onChange={this.setSecret} className="form-control" placeholder="Код"/>
                    </div>
                    }

                    {this.state.authFail && <div className="form-group">
                      <div className="alert alert-danger">
                        <button type="button" className="close" onClick={() => this.setState({authFail: false})} data-dismiss="alert" aria-hidden="true">&times;</button>
                    ...
                    <div className="form-group">
                      {this.state.codeWasRequested === false &&
                        <input type="submit" onClick={this.getCode} className="btn btn-block btn-kpi-id" value="Отримати код"/>
                      }

                      {this.state.codeWasRequested === true &&
                        <input type="submit" onClick={this.authorize} className="btn btn-block btn-kpi-id" value="Вхід"/>
                      }
                    </div>
