...
namespace WebApplication1
{
    public class DictionaryManager
    {
        ...
        public DataView GetCathedra()
        {
            ...
            SqlDataSource val = new SqlDataSource();
            //val.DataSourceMode = SqlDataSourceMode.DataReader;
            val.ConnectionString = WebConfigurationManager.ConnectionStrings["DB"].ToString();
            //val.SelectCommand = "SELECT Name, DcSubdivisionId FROM [DcSubdivision] WHERE [DcSubdivisionTypeId]=30 ORDER BY Name";
            val.SelectCommand = "select s.Name, s.DcSubdivisionId from RtResponsible r, DcSubdivision s where r.DcSubsystemId = 1 and r.UserAccountId = " + userId + "and r.DcSubdivisionId = s.DcSubdivisionId ORDER BY Name";

            //var reader = (SqlDataReader)val.Select(DataSourceSelectArguments.Empty);
            //DataView resp = new DataView();

            //if (reader != null && reader.HasRows)
            //{
            //    reader.Read(); 
            //    resp = (DataView)val.Select(DataSourceSelectArguments.Empty);
            //}
            return (DataView)val.Select(DataSourceSelectArguments.Empty);
