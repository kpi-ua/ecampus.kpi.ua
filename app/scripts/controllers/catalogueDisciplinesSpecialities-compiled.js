...
(function () {
  ...
  angular.module('ecampusApp').controller('CatalogueDisciplinesCpesialitiesCtrl', CatalogueDisciplinesCpesialitiesCtrl);
  ...
  function CatalogueDisciplinesCpesialitiesCtrl($scope, api, sharedFiltersData, permission) {
    ...
    function loadFaculties() {
      ...
      var method = 'GET';
      api.execute(method, url).then(function (response) {
    ...
    function loadSubdivisionsAccordingToPermission() {
      ...
      var subsystemUrl = 'Subdivision/subsytem/' + SUBSYSTEM_ID + '/cathedras';
      var kpiUrl = 'Subdivision/subsytem/' + KPI_ID + '/cathedras';
      var profTrainsUrl = 'StudyOrganization/ProfTrains/';
      var queriesArray = [subsystemUrl, kpiUrl, profTrainsUrl];
      var method = 'GET';
      var apiExecuteGet = api.execute.bind(api, method);
      Promise.all(queriesArray.map(apiExecuteGet)).then(function (_ref) {
        var _ref2 = _toArray(_ref),
            subsystemCathedras = _ref2[0],
            kpiCathedras = _ref2[1],
            profTrains = _ref2[2],
            rest = _ref2.slice(3);

        var allPermissions = subsystemCathedras.concat(kpiCathedras);

        var availableCathedrasId = allPermissions.map(function (cathedra) {
          return cathedra.id;
        });

        var availableProfTrains = profTrains.filter(function (profTrain) {
          return availableCathedrasId.indexOf(profTrain.subdivision.id) !== -1;
        }).filter(function (profTrain) {
          return profTrain.specialtyCode.indexOf('.') !== -1;
        });

        $scope.allSpecialities = availableProfTrains;

        $scope.specialities = uniqueSpecialities(availableProfTrains).sort(sortNames).map(function (speciality) {
          return _extends({}, speciality, {
    ...
    function filterSpecialities(allSpecialities, subdivisionId) {
      ...
      return allSpecialities.filter(function (element) {
        return element.subdivision.id === subdivisionId;
      });
    }
    ...
    function loadSpecialities(subdivisionId) {
      ...
      api.execute('GET', url).then(function (response) {
        ...
        $scope.specialities = uniqueSpecialities(specialitiesWithOkr).sort(sortNames).map(function (speciality) {
          return _extends({}, speciality, {
            nameToDisplay: speciality.okr.name + ' - ' + speciality.specialtyCode + ' ' + speciality.name
          });
        });
    ...
    $scope.saveDisc = function (editableObj, objDisc) {
      ...
      var sendDisc = new DisciplineModel(ProfTrainTotalId, ProfTrainTotalSubdivisionId, DcSpecializationId, Component, Discipline8, Shifr, OutCredit, Description, Actuality);

      api.execute(method, url, sendDisc).then(function (response) {
    ...
    $scope.removeDisc = function (currentRowDisc) {
      if (confirm('Ви впеврені що хочете видалити поточну дисципліну?')) {
        var url = 'StudyOrganization/Discipline/' + currentRowDisc.rtDisciplineId;
    ...
    function DisciplineModel(ProfTrainTotalId, ProfTrainTotalSubdivisionId, DcSpecializationId, Component, Discipline8, Shifr, OutCredit, Description, Actuality) {
    ...
    function DisciplineNewModel(Name, NameShort, Abbreviation, UserAccount, DcOKRId, DcSubdivisionWhoId) {
      this.Name = Name;
    ...
    $scope.saveDisciplineModal = function () {
      ...
      var sendDisc = new DisciplineNewModel(Name, NameShort, Abbreviation, UserAccount, DcOKRId, DcSubdivisionWhoId);
    ...
    $scope.addDescription = function () {
      ...
      var sendDisc = new DisciplineModel(ProfTrainTotalId, ProfTrainTotalSubdivisionId, DcSpecializationId, Component, Discipline8, Shifr, OutCredit, Description, Competence, Knowledge, Skill);
...
//# sourceMappingURL=catalogueDisciplinesSpecialities-compiled.js.map
